...
void BaseTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle,
  ...
  // Upstream gates the header clock on a clock chip being present; this fork also shows it
  // on devices without one, reading the system clock, so the test is "can we render a time
  // at all" rather than "is there an RTC". The band reservation below depends only on the
  // setting, never on whether a time exists yet — see ReaderUtils.
  char clockProbe[9];
  const bool showHeaderClock =
      halClock.formatCurrentTime(clockProbe, sizeof(clockProbe), SETTINGS.clockUtcOffsetQ, SETTINGS.clockFormat == 1) &&
...
void BaseTheme::drawTopStatusBarClock(const GfxRenderer& renderer, int topY, const char* previewTime,
  ...
  if (timeText == nullptr) {
    if (!halClock.formatCurrentTime(timeBuf, sizeof(timeBuf), SETTINGS.clockUtcOffsetQ, SETTINGS.clockFormat == 1)) {
      return;
    }
    timeText = timeBuf;
  }
