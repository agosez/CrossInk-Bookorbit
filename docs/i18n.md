...
English is the source of truth. Add new `STR_*` keys to `lib/I18n/translations/english.yaml`, then propagate translations to the other YAML files when possible. If a file is missing a key, the generator falls back to that language's base language, which is English unless the file names another one (see `_base_language`).
...
- `_base_language` is optional and names another `_language_code` to inherit from instead of English. Use it only for a close variant of an existing language: `CAV` (Valencian) declares `CA`, and `PT` (Brazilian Portuguese) declares `PT2`. It has two effects. A missing key falls back to the base language rather than to English, which is what a reader of a variant would rather see. And any string the file spells exactly like its base is stored once instead of twice, which is worth several kilobytes of flash per pair. Chains must end at English; a cycle is a generator error.
...
Missing strings will fall back to the language's base (English by default), but a new language should not ship with large untranslated areas unless that is intentional.
...
The flat blob plus offset-table layout keeps lookup simple and avoids a large table of individual C string pointers. A language stores nothing for a string its base language already spells the same way: the offset is bit 15 (`0x8000`) alone, and `I18n::get()` follows the base chain until it reaches the language that holds the bytes. Most languages base on English directly, so that is one hop; a variant such as Valencian can take two (Valencian, then Catalan, then English).
...
2. Check for languages left on their fallback where a real translation was expected:
