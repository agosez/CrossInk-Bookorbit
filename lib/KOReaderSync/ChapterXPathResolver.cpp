...
std::string buildTextXPointer(const int spineIndex, const std::vector<PathSegment>& path, const int textNodeIndex,
                              const size_t charOffset) {
  std::string xpath = "/body/DocFragment[" + std::to_string(spineIndex + 1) + "]/body";
  for (const auto& segment : path) {
    xpath += "/" + segment.name + "[" + std::to_string(segment.index) + "]";
...
bool ChapterXPathResolver::findHighlightXPointers(const std::shared_ptr<Epub>& epub, const int spineIndex,
